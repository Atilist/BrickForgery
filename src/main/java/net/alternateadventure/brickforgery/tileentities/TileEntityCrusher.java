...
public class TileEntityCrusher extends TileEntityBase implements InventoryBase {
    ...
    private boolean canAcceptRecipeOutput() {
        ...
        if (!canAcceptByproduct(crushingOutput.byproduct)) return false;

        if (this.inventory[1] == null) {
            return true;
        } else if (!this.inventory[1].isDamageAndIDIdentical(crushingOutput.tieredMachineRecipeData.output)) {
            return false;
        } else if (this.inventory[1].count < this.getMaxItemCount() && this.inventory[1].count < this.inventory[1].getMaxStackSize()) {
            return true;
        } else {
            return this.inventory[1].count < crushingOutput.tieredMachineRecipeData.output.getMaxStackSize();
        }
    ...
    public void craftRecipe() {
        if (this.canAcceptRecipeOutput()) {
            TierAndByproductOutput crushingOutput = CrushingRecipeRegistry.getInstance().getResult(inventory[0].itemId);
            if (this.inventory[1] == null) {
                this.inventory[1] = crushingOutput.tieredMachineRecipeData.output.copy();
            } else if (this.inventory[1].itemId == crushingOutput.tieredMachineRecipeData.output.itemId) {
            ...
            {
                this.inventory[2] = crushingOutput.byproduct;
            }
            else
            {
                this.inventory[2].count += crushingOutput.byproduct.count;
            }
        }
    }

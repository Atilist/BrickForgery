...
public class PrimitiveSlicer extends LazyBlockTemplate {
    public PrimitiveSlicer(Identifier identifier, Material material, float hardness, BlockSounds blockSounds) {
    ...
    public boolean canUse(Level level, int x, int y, int z, PlayerBase playerBase) {
        ...
        if (item.itemId != ItemBase.leather.id) return false;
        item.count--;

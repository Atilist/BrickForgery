...
public class WasherBase extends LazySimpleMachine {
    private Random rand = new Random();
    public int tier;
    public WasherBase(Identifier identifier, Material material, float hardness, BlockSounds blockSounds, int tier) {
    ...
    public boolean canUse(Level world, int x, int y, int z, PlayerBase player) {
        ...
        if (tileEntity instanceof TileEntityWasher tileEntityWasher)
            GuiHelper.openGUI(player, Identifier.of(TileEntityListener.MOD_ID, "gui_washer"), tileEntityWasher, new ContainerWasher(player.inventory, tileEntityWasher));
    ...
    public void onBlockRemoved(Level arg, int i, int j, int k) {
        ...
        TileEntityWasher washer = (TileEntityWasher) arg.getTileEntity(i, j, k);

        for(int var6 = 0; var6 < washer.getInventorySize(); ++var6) {
            ItemInstance var7 = washer.getInventoryItem(var6);
            if (var7 != null) {

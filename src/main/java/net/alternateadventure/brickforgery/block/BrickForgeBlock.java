...
import net.alternateadventure.brickforgery.screenhandler.BrickForgeScreenHandler;
...
public class BrickForgeBlock extends LazySimpleMachineBlockTemplate implements MechanicalDevice {
    private final Random rand = new Random();

    public TierEnum tier;
    private int inputSideTexture;

    public BrickForgeBlock(Identifier identifier, Material material, float hardness, BlockSoundGroup blockSounds, TierEnum tier) {
    ...
    public boolean onUse(World world, int x, int y, int z, PlayerEntity player) {
        ...
        if (tileEntity instanceof BrickForgeBlockEntity brickForgeBlockEntity)
            GuiHelper.openGUI(player, Identifier.of(BlockEntityListener.MOD_ID, "gui_brick_forge"), brickForgeBlockEntity, new BrickForgeScreenHandler(player.inventory, brickForgeBlockEntity));
        return true;
    }

    @Override
    protected BlockEntity createBlockEntity() {
        return new BrickForgeBlockEntity();
    }

    public void onBreak(World arg, int i, int j, int k) {
        BrickForgeBlockEntity slicer = (BrickForgeBlockEntity) arg.getBlockEntity(i, j, k);
